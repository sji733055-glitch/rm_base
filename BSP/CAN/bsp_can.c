...
Can_Device *BSP_CAN_Device_Init(Can_Device_Init_Config_s *config)
{
    ...
    // 分配事件标志
    int flag_index =
        bus_manager->device_count + (bus_manager - can_bus_managers) * MAX_DEVICES_PER_CAN_BUS;
    if (flag_index < 32)
    {
    ...
    {
        HAL_CAN_ActivateNotification(config->can_handle,
...
osal_status_t BSP_CAN_SendDevice(Can_Device *device)
{
    ...
    // 发送数据
    status = HAL_CAN_AddTxMessage(device->can_handle, &device->txconf, device->tx_buff,
                                  &device->tx_mailbox);
    ...
    if (device->tx_mode == CAN_MODE_IT)
    {
        ...
        // 等待发送完成事件
        osal_status_t wait_status = osal_event_wait(
            &global_can_event, tx_event_flag, OSAL_EVENT_WAIT_FLAG_OR | OSAL_EVENT_WAIT_FLAG_CLEAR,
...
osal_status_t BSP_CAN_ReadSingleDevice(Can_Device *device, osal_tick_t timeout)
{
    ...
    if (device->rx_mode == CAN_MODE_BLOCKING)
    {
        ...
        if ((device->tx_id & 1))
        {
            status = HAL_CAN_GetRxMessage(device->can_handle, CAN_RX_FIFO0, &rx_header, rx_data);
        }
        else
        {
            status = HAL_CAN_GetRxMessage(device->can_handle, CAN_RX_FIFO1, &rx_header, rx_data);
        }
...
uint32_t BSP_CAN_ReadMultipleDevice(Can_Device **devices, uint8_t device_count, osal_tick_t timeout)
{
    ...
    for (int i = 0; i < device_count; i++)
    {
        if (devices[i] != NULL)
        {
            combined_event_flags |= devices[i]->eventflag;
        }
    }
