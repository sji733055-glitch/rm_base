...
void DMMotorDecode(DMMOTOR_t *motor)
{
    ...
    if (delta_position > 180) {motor->measure.total_round--;} 
    else if (delta_position < -180) {motor->measure.total_round++;}         
...
void DMMotorcontrol(void){
    ...
    // 检查dmm_motor_list是否已初始化
    if (dmm_motor_list == NULL) {return;}

    for (size_t i = 0; i < DM_MOTOR_CNT; ++i){
        if (dmm_motor_list[i].can_device == NULL){continue;}      
        motor = &dmm_motor_list[i];
        if (motor == NULL) {continue;}
        if (offline_module_get_device_status(motor->offline_index)==1 || motor->stop_flag == MOTOR_STOP) // 如果电机处于离线状态,发送0 若该电机处于停止状态,直接将buff置零
        {
            control_output = 0;
